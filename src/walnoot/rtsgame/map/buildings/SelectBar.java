...
public class SelectBar {
	...
	public SelectBar(Map map){
		this.map = map;
		try {
			bar = ImageIO.read(this.getClass().getResource("/selectbar.png"));
		} catch (Exception e) {
	...
	public void update(Point mousePos, boolean mouseIsDown, Point translation){
		int indexSelected;
		if(isOnButtons(mousePos)){
		...
		for(BuildingButtons b:buttons){
			b.update(mousePos,mouseIsDown,translation,isInBar(mousePos));
		}
